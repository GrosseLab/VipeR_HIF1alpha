...
summarizeToGene.list <- function(object,
  ...
  which(tx2gene$gene == x)
  which(geneId == x)
  which(tx2gene3$gene == x)
 
  tx2gene[tx2gene$gene == x,]
   
  tx2gene3 <- data.frame(cbind(txId,'gene'=as.character(geneId)))
  abundanceMatTx[which(tx2gene3$gene == x),]
  
  abundanceMatTx[as.character(tx2gene3[tx2gene3$gene == x,]$tx),]
  abundanceMatTx[as.character(tx2gene3[tx2gene3$gene == x,]$tx),]
  
  
  tx2gene3[tx2gene3$gene == x,]
  tx2gene[tx2gene$gene == x,]
  
  # summarize abundance and counts
  message("summarizing abundance")
  abundanceMatTx[LDAHanno$transcript_id,]
  
  tmpgeneId <- geneId[geneId == LDAHanno$gene_id[1]]
  tmpgeneId <- droplevels(tmpgeneId)
  
  rowsum(abundanceMatTx[LDAHanno$transcript_id,], tmpgeneId)
  
  x <- LDAHanno$gene_id[1]
  sum(geneId == LDAHanno$gene_id[1])
  
  
  f.input2(as.character(TrGe2$tx),as.character(TrGe2$gene))
  
  abundanceMat2 <- sapply(unique(as.character(geneId)), function(x) sum( abundanceMatTx[tx2gene[tx2gene$gene == x,]$tx,] ) )
